...
for row in resultsAllCode:
    ...
    selectCountRowsByCode = "SELECT COUNT(*) FROM TmpMinimumTablePoints WHERE Code ='" + code + "'"
    countOfRows = cursor.execute(selectCountRowsByCode).fetchone()[0]
    
    if (countOfRows > 1):
        selectRowsByCode = "SELECT Points, Year FROM TmpMinimumTablePoints WHERE Code = '" + code + "'"
        ...
        insert = "INSERT INTO TmpTableMinPredictionsByCode(Id, MinPrediction, Code, Year) VALUES('" + str(uuid.uuid4()).upper() + "', " + str(int(point)) + ", '" + code + "', '" + str(int(now.year) + 1) + "')"
        cursor.execute(insert)
        cursor.commit()
    elif (countOfRows == 1):
        selectPointsByCode = "SELECT Points, Year FROM TmpMinimumTablePoints WHERE Code = '" + code + "'"
        point = cursor.execute(selectPointsByCode).fetchone()[0]
        year = cursor.execute(selectPointsByCode).fetchone()[1]
        insert = "INSERT INTO TmpTableMinPredictionsByCode(Id, MinPrediction, Code, Year) VALUES('" + str(uuid.uuid4()).upper() + "', " + str(point) + ", '" + code + "', '" + str(int(now.year) + 1) + "')"
        cursor.execute(insert)
        cursor.commit()
    else:
        insert =  "INSERT INTO TmpTableMinPredictionsByCode(Id, MinPrediction, Code, Year) VALUES('" + str(uuid.uuid4()).upper() + "', " + str(0) + ", '" + code + "', '" + str(int(now.year) + 1) + "')"
        cursor.execute(insert)
        cursor.commit()
